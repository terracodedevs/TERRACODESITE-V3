const ProcessCardProps = [
  {
    ...
    icon: "iconamoon:search-bold.svg",
    imageSrc: "Vector.svg",
  ...
  return (
    <div className="flex flex-row gap-5">
      {ProcessCardProps.map((process, index) => (
        <div
          key={index}
          className="group w-full relative rounded-3xl bg-[#1B1B1B] h-[408px] overflow-hidden text-center text-2xl text-neutral-500 hover:text-white text-darkorange font-lufga" style={{ fontFamily: 'Lufga, sans-serif' }}
        >
          {/* Bottom gradient line on hover */}
          <div className="absolute bottom-0 left-0 w-full h-[1.5px] opacity-0 group-hover:opacity-70 transition-opacity duration-300 bg-gradient-to-r from-transparent via-amber-500 to-transparent pointer-events-none" />
          ...
          <div className="absolute top-[32px] left-[24px] w-[264px] flex flex-col items-start justify-start gap-4">
            <div className="self-stretch relative text-[32px] tracking-[0.04px] leading-10 text-left">
              {process.title}
            </div>
            <div className="self-stretch relative text-xl tracking-[0.04px] leading-6 text-left">
